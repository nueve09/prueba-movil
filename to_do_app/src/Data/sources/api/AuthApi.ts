...
const findUser = (email: string, password: string): User | string => {
  const user = usersData.find((item: User) => item.email === email);

  if (!user) return 'not found';

  if (user.password !== password) return 'bad credentials';
